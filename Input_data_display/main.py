...
machine.freq(240000000) #เรียกใช้ฟังก์ชันในโมดูล 'machine' เพื่อกำหนดความถี่ของตัวประมวลในบอร์ด
...
print("Starting AMG addr detected on I2C...") #แสดงข้อความว่ากำลังเริ่มต้นการตรวจจับของอุปกรณ์ AMG8833ผ่านการเชื่อมต่อ I2C
print("-" * 50) #แสดงขีดจำนวน 50 ขีดในการคั่น
...
sta_if.active(True)
sta_if.connect('BS-Labs_Guest', 'thanktobs')
    ...
    try:
        ...
        for h in range(8): #วนลูปตามตัวแปร h ซึ่งจะเป็นตัวเลขตั้งแต่ 0 ถึง 7 เพื่อแทนแถว (row) ของพิกเซลในตาราง 8x8
            for w in range(8): #วนลูปตามตัวแปร w ซึ่งจะเป็นตัวเลขตั้งแต่ 0 ถึง 7 เพื่อแทนคอลัมน์ (column) ของพิกเซลในตาราง 8x8
                ...
                # if(t < 27.99): TFTCOLOR = TFTColor(0xEB, 0x83, 0x34)    #(สีน้ำตาลอ่อน)
                # elif(t < 28.51): TFTCOLOR = TFTColor(0xEB, 0xC3, 0x34)  #(สีน้ำตาลอ่อน
                # elif(t < 28.99): TFTCOLOR = TFTColor(0xE4, 0xEB, 0x34)  #(สีเหลืองอ่อน)
                # elif(t < 29.51): TFTCOLOR = TFTColor(0xBB, 0xEB, 0x34)  #(สีเหลืองอ่อน)
                # elif(t < 29.99): TFTCOLOR = TFTColor(0xB1, 0xEB, 0x34)  #(สีเหลืองอ่อน)
                # elif(t < 30.51): TFTCOLOR = TFTColor(0x34, 0xEB, 0x6E)  #(สีเขียวอ่อน)
                # elif(t < 30.99): TFTCOLOR = TFTColor(0x34, 0xEB, 0xBF)  #(สีเขียวอ่อน)
                # elif(t < 31.55): TFTCOLOR = TFTColor(0x34, 0xEB, 0xEB)  #(สีฟ้าอ่อน)
                # elif(t < 31.99): TFTCOLOR = TFTColor(0x53, 0xD5, 0x53)  #(สีเขียวอ่อน)
                # elif(t < 32.55): TFTCOLOR = TFTColor(0x34, 0xB1, 0xEB)  #(สีฟ้าอ่อน)
                # elif(t < 32.99): TFTCOLOR = TFTColor(0x34, 0x8E, 0xEB)  #(สีฟ้าอ่อน)
                # elif(t < 33.55): TFTCOLOR = TFTColor(0x34, 0x70, 0xEB)  #(สีฟ้าอ่อน)
                # elif(t < 33.55): TFTCOLOR = TFTColor(0x34, 0x53, 0xEB)  #(สีฟ้าอ่อน)
                # elif(t < 33.99): TFTCOLOR = TFTColor(0x1D, 0x3F, 0xE7)  #(สีฟ้าเข้ม)
                # elif(t < 34.55): TFTCOLOR = TFTColor(0x00, 0x00, 0xFF)  #(สีน้ำเงิน)
                # elif(t < 34.99): TFTCOLOR = TFTColor(0xD9, 0x27, 0x27)  #(สีแดงอ่อน)
                # elif(t > 35.00): TFTCOLOR = TFTColor(0x02, 0x02, 0xC4)  #(สีน้ำเงินเข้ม)
