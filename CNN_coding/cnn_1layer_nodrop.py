...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        self.model = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(4), padding=0),  # คอนโว = layerเเรก
            torch.nn.BatchNorm2d(32), # ปรับค่าให้มี mean = 0 และ std = 1 ในแต่ละชั้น
            torch.nn.Dropout(),  # ลด overfitting โดยการปิดบางเเถว ในการเทรน
            torch.nn.ReLU(),  # Rectified Linear Unit ถ้าค่าเป็นบวกให้เอาค่านั้นไป ถ้าเป็นลบให้เอาค่า 0 ไป
            torch.nn.MaxPool2d(kernel_size=3, stride=1),  # เลือกค่าที่มากที่สุดใน 2x2 จะได้ 4x4

            # torch.nn.Conv2d(in_channels=32, out_channels=16, kernel_size=(3), padding=0),  # เหลือ 2x2 output 16x2x2
            ...
            torch.nn.Flatten(),
            torch.nn.Linear(32 * 3 * 3, 2),  # ชั้น Fully Connected Layer หลังจาก Linear: 4 -> 32
            # torch.nn.ReLU(),
...
df = pd.read_csv(r"C:\read_thermal\SumAllCase\data_ok_v2.csv")
...
dropout_rate = 0.5  # ความน่าจะเป็นในการปิดการทำงานของโนด
...
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate,
                             weight_decay=weight_decay)  # Adaptive Moment Estimation ปรับ lerning late ให้เหมาะสมกับแต่ละรอบ ลดการเกิด overfitting
...
train_loader = data_utils.DataLoader(train_set, batch_size=batch_size,
                                     shuffle=True)  # สร้าง dataloader สำหรับ train set
...
for i in range(rounds):
    file_name = f'./model_save/CNN--Batch_size-{batch_size}--Learning_rate-{learning_rate}--Layer_size--{layer_size}--Epochs--{num_epochs}--round--{rounds}--{dt_string}--Dropout_rate--{dropout_rate}--Weight_decay--{weight_decay}'
...
plt.title(
    f"CNN=Learning_late={learning_rate},batch_size={batch_size},epoch={num_epochs},\nweight_decay={weight_decay},Layer_size={layer_size},cnnOptim={type(optimizer).__name__},Dropout={dropout},Enable BatchNorm2d")
