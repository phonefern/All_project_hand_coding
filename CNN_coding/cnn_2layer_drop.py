...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        self.model = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(2), padding=0),  # คอนโว = layerเเรก
            ...
            torch.nn.Flatten(),
            torch.nn.Linear(16 * 4 * 4, 2),  # ชั้น Fully Connected Layer หลังจาก Linear: 4 -> 32
            # torch.nn.ReLU(),
...
df = pd.read_csv(r"C:\read_thermal\SumAllCase\balanced_dataset\split_50k_3.csv")
...
for i in range(rounds):
    file_name = f'./model_save/CNN--Batch_size-{batch_size}--Learning_rate-{learning_rate}--Layer_size--{layer_size}--Epochs--{num_epochs}--round--{rounds}--{dt_string}--Dropout--{0.5}'
...
plt.title(
    f"CNN=Learning_late={learning_rate},batch_size={batch_size},epoch={num_epochs},\nweight_decay={weight_decay},Layer_size={layer_size},cnnOptim={type(optimizer).__name__},Dropout=0.5")
