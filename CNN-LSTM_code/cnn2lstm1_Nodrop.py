...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = torch.nn.Conv2d(in_channels=32, out_channels=16, kernel_size=2, stride=1, padding=0)
        self.batch2 = torch.nn.BatchNorm2d(16)  # **optional
...
input_size = 4 * 4 * 16  # for 2 layers CNN
...
num_epochs = 200  # [50, 100, 150, 250, 300]
# num_epochs = []
...
for i in range(rounds):
    file_name = f'./model_save/CNN-LSTM--Batch_size-{batch_size}--Learning_rate-{learning_rate}--Layer_size_LSTM--{layer_size}-Layer_size_cnn-{layer_cnn}-Epochs--{num_epochs}--Dropout--{dropout}--round--{rounds}--Hidden_size--{hidden_size}'
...
plt.ylabel('Actual')
plt.title(f"CNN-LSTM_Confusion Matrix(Test Normal) - Epochs: {num_epochs}, Learning Rate: {learning_rate}\n"
